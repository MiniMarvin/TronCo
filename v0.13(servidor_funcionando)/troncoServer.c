...
int main(int argc, char **argv){
    ...
    jogador[0].x = 0; jogador[0].y = 0; jogador[0].dir = buff_dir[0] = UP;
    jogador[1].x = 0; jogador[1].y = SIZEY - 1; jogador[1].dir = buff_dir[1] = DOWN;
    jogador[2].x = SIZEX - 1; jogador[2].y = 0; jogador[2].dir = buff_dir[2] = RIGHT;
    jogador[3].x = SIZEX - 1; jogador[3].y = SIZEY - 1; jogador[3].dir = buff_dir[3] = LEFT;  
   ...
   while(!sair){
        ...
        if(retorno.status == MESSAGE_OK){
            ...
            if(jogador[id].x >= SIZEX || jogador[id].y >= SIZEY ||
              ...
              pontuacao = cleanMatrix(matrizJogo, id);
