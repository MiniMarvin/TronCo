...
int main(int argc, char **argv){
    ...
    jogador[1].x = 0; jogador[1].y = SIZE - 1; jogador[1].dir = DOWN;
    jogador[2].x = SIZE - 1; jogador[2].y = 0; jogador[2].dir = LEFT;
    jogador[3].x = SIZE - 1; jogador[3].y = SIZE - 1; jogador[3].dir = LEFT;  
...
void cleanMatrix(char matrix[SIZE][SIZE], int player) {
  int i, j;
  for (i = 0; i < SIZE; i++) {
    for (j = 0; j < SIZE; j++) {
